class SceneManager {
    ...
    loadLevel(levelIndex, transition, title, dead, end) {
        ...
        if (oldLevelIndex && !this.savedState[oldLevelIndex]) {
            this.savedState[oldLevelIndex] = this.game.entities
            .filter(entity => !(entity instanceof Knight))
            .map(entity => Object.assign(Object.create(Object.getPrototypeOf(entity)), entity));
        ...
        // if(transition) {
        //     this.game.addEntity(new TransitionScreen(this.game, this.levelIndex, dead, end));
        //     return;
        // }

        if (this.savedState[this.levelIndex]) {
            this.game.entities = this.savedState[this.levelIndex]
                .filter(entity => !(entity instanceof Knight))
                .map(entity => 
                    Object.assign(Object.create(Object.getPrototypeOf(entity)), entity)
            );
        } else {
            if (this.level.text) {
